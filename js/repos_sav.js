...
(function($, undefined) {
  ...
  var Urls = {
    "team_url": "repos/getJson.php?s=teams",
    "repo_url": "repos/getJson.php?s=repos",
    "member_url": "repos/getJson.php?s=members",
    "group_url": "repos/getJson.php?s=groups"
  };
  ...
  function createRepoForCarousel(repo, active, image) {
    ...
    // Compute number of star
    var $star = ($("<ul>").addClass("list-inline list-stars pull-right").text(repo.stargazers_count + ' ')).appendTo($repo_head);
    ...
    for (var i = _star_active; i < _max_star; i++)
      $star.append($("<li>").html('<i class="github-star"></i>'));
  ...
  function getNumMembers(page, numMembers) {
    ...
    var membersUri = Urls["member_url"] + "&page=" + pageNum;
    $.getJSON(membersUri, function(result) {
      if (result && result.length > 0) {
        numMembers += result.length;
        getNumMembers(pageNum + 1, numMembers);
        _members.push(result);
      } else {
                         ...
                         for (var group of groups) {
                           ...
                           for (var tname of group.teams) {
                             if (tname && MapTeams[tname]) {
                               ...
                               var $repoAll = $("<div>").addClass("row").append($("<h3>").text(team.name));
                               ...
                               $repoRow.append($("<h3>").text(team.name));
                               ...
                               for (var rname of team.repo) {
                                 if (rname && MapRepos[rname]) {
                                   ...
                                   $repoAll.append(addRepo(repo));
                                   repo_count++;
                                   // console.log("    |-> Add repo " + rname);

                                   // TODO: order the teams so that the last modified is first
                                 }
                               }
                               $repoRow.appendTo($repoGroup);
                               $repoAll.appendTo($teamAll);
                             }
